...
mapCountryData(n, 
               ...
               mapTitle="World Map for Happiness Score 2015-2022 \nMost unhappy countries are located in Sub-Saharan Africa and South-East Asia",
...
ggplot(top10_bottom10_countries) +
  ...
  labs(title = "There is a high correlation between higher GDP per capita and higher \nhappiness score, but there can also be lower happiness in higher \nGDP per capita countries.",
       subtitle = "Happiness vs. GDP per capita for Top 5 and Bottom 5 countries in 2022.",
       x = "GDP per capita",
       y = "Happiness score") +
...
ggplotly(region_level, tooltip = c("country", "happiness_score"))%>%
  layout(title = list(text = paste0('Country-wise happiness trends in world regions',
                                    '<br>',
                                    '<sup>',
                                    'Middle East and North African countries have the highest disparity in Happiness score',
                                    '</sup>')))
```
...
ggplot(joined_data, aes(x = co2_emissions_per_capita, y = happiness_score, size = co2_emissions)) +
  ...
  labs(
    ...
    subtitle = paste("Happiness Score vs CO2 Emissions Per Capita (Log Scale), r =", round(cor_co2, 2)),
...
ggplot(whr2015_2022, aes(x = health_life_expectancy, y = happiness_score)) +
  ...
  labs(
    ...
    subtitle = "Happiness Score relative to Life Expectancy",
    x = "Life Expectancy",
    y = "Happiness Score"
  ) +
  theme_classic()
...
ggplot(joined_data, aes(x = density_p_km2, y = happiness_score, size = urban_population)) +
  ...
  labs(
    ...
    subtitle = paste("Happiness Score vs Population Density, r =", round(cor_urban, 2)),
...
ggplot(joined_data, aes(x = urban_population_per_capita, y = happiness_score, size = urban_population)) +
  ...
  labs(
    ...
    subtitle = paste("Happiness Score vs Urban Population Per Capita, r =", round(cor_urban, 2)),
...
ggplot(forecast_df, aes(x = year, y = value)) +
  geom_line(size = 1, color = "#99CC99") +
  geom_point(color = "#99CC99") +
  geom_line(size = 1, data = forecast_df[length(yearly_data$year) + 1:nrow(forecast_df), ], color = "#FF9999") +
  geom_point(data = forecast_df[length(yearly_data$year) + 1:nrow(forecast_df), ], color = "#FF9999") +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "#FF9999", alpha = 0.2) +
  scale_x_continuous(breaks = forecast_df$year) +
  labs(title = "The overall worldwide happiness score increased from 2015 to 2022",
       subtitle = "The happiness score is also projected to increase in 2023 and 2024",
...
ggplot(region_yearly_data, aes(x = year, y = avg_happiness_score, group = region, color = region)) +
  ...
  theme_minimal() +
  labs(title = "The happiness score for each region remained relatively stable from 2015 to 2022",
       subtitle = "The region that earned a largest increase in happiness score is Central and Eastern Europe",
       x = "Year",
...
ggplot(whr2015_2022, aes(x = happiness_score, y = factor(year), fill = region)) +
  ...
  labs(title = "Distribution of Happiness Scores of Different Countries by Region and Year", x = "Happiness Score", y = "Year",
       subtitle = "Western Europe always ranks first while Sub-Saharan Africa and South Asia report \nlower scores") +
...
ggplot(whr2015_2022, aes(x = economy_gdp_per_capita, y = happiness_score)) +
  ...
  labs(title = "2D Density Plot of GDP per Capita vs. Happiness Score",
       subtitle = "A positive correlation exists with the highest density observed at mid-range values",
...
ggplot(whr2015_2022, aes(x = reorder(region, happiness_score, FUN = mean), y = health_life_expectancy, fill = region)) +
  ...
  labs(title = "Violin Plot of Life Expectancy by Region",
       subtitle = "Higher life expectancy and uniform distributions are in regions \nwith higher happiness scores",
       x = "Region",
       y = "Life expectancy") +
  theme_minimal() +
  theme(
...
ggplot(categorise_trust, aes(x = trust_category, y = happiness_score, color = trust_category)) +
  ...
  labs(title = "Countries with high trust in government seem to be mostly happy, \nwhile there is more disparity with lower levels of trust",
       subtitle= "Happiness Score by Trust in Government",
       x = "Trust in Government",
       y = "Happiness Score") +
  theme_minimal() +
  scale_color_brewer(palette = "Pastel1")+ # Optional: This adds a color palette
  theme(legend.position = "none")
```
